...
import com.cm.neuronflow.services.GeminiToolService;
...
public class InternalToolController {
    ...
    private final GeminiToolService geminiToolService;

    // The base URL of your backend (e.g., http://localhost:8080 or https://api.neuronflow.com)
    @Value("${app.backend.url:http://localhost:8080}")
    private String backendUrl;

    public InternalToolController(GeminiToolService geminiToolService) {
        this.geminiToolService = geminiToolService;
    }
    ...
    public ResponseEntity<Map<String, String>> generateImage(@RequestBody Map<String, String> request) {
        ...
        String imageId = geminiToolService.generateAndCacheImage(prompt);
