...
public class GeminiToolService {
    ...
    private static final String IMAGE_MODEL_ID = "gemini-3.1-flash-image";
    private static final String CODE_MODEL_ID = "gemini-3.5-flash";
    ...
    public GeminiToolService(Client geminiClient, ObjectMapper objectMapper, RedisTemplate<String, Object> redisTemplate) {
    ...
    /**
     * 1. Generates an image using Gemini Image generator model,
     *    saves Base64 to Redis, and returns UUID.
     */
    public String generateAndCacheImage(String prompt) {
        log.info("Generating image via Gemini for prompt: {}", prompt);
        try {
            ...
            GenerateImagesResponse response = geminiClient.models.generateImages(
                    IMAGE_MODEL_ID,
                    prompt,
                    config
            );

            if (response.generatedImages() == null || response.generatedImages().isEmpty()) {
                throw new IllegalStateException("No image generated by Gemini API");
            }
            ...
            throw new NeuronFlowException("Failed to generate image with Gemini: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    /**
     * 2. Generates React UI Code using Gemini
     */
    public Map<String, String> generateCanvasCodeWithDescription(String prompt, String interactivityDescription) {
        log.info("Generating React canvas code via Gemini 3.5 Flash for prompt: {}", prompt);
        ...
        String userPrompt = "Create an interactive React educational visualization for: " + prompt +
        ...
        try {
            Schema responseSchema = Schema.builder()
                    ...
                    .properties(Map.of(
                            ...
                            "walkthrough", Schema.builder().type(STRING).build()
                    ))
            ...
            log.error("Failed to generate UI code with Gemini: {}", e.getMessage(), e);
            throw new NeuronFlowException("Failed to generate UI code with Gemini: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
